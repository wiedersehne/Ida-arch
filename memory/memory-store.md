...
The capture pipeline produces three categories of persistent knowledge: per-entity data findings, non-quantitative project facts, and user behavioral profiles. These need to survive across sessions, beyond the lifetime of a single chat or summarization pass.
...
**Primary key for upsert:** `(agent_id, name, scope, project_id, org_id, user_id)`. All reads and writes use this composite key. Two records with the same name but different `scope` or `project_id` are distinct rows.
...
Every `set_object` or `update_object` call increments `version`. This tracks how many times a record has been synthesised — an `entity_nnm101` record with `version=5` has been merged across five consolidation passes.